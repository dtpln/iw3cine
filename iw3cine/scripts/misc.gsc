...
{
    vh = args[0];
    self iPrintLn( "[" + level.HIGHLIGHT_COLOR + "IW3Cine^7]Setting viewmodel to: " + vh );
...
toggle_freeze()
{
    ...
    for ( i = 0; i < level.players.size; i++ )
    {
        ...
        self iPrintLn( "[" + level.HIGHLIGHT_COLOR + "IW3Cine^7] Frozen bots: " + level.COMMAND_COLOR + bool( level.BOT_FREEZE ) );
...
spawn_fx( args )
{
    ...
    self iPrintLn( "[" + level.HIGHLIGHT_COLOR + "IW3Cine^7]Spawned fx: " + fx );
}
...
about() // Fix
{
    ...
    text[1] = elem( -33, 1, "qerFont",    "Ported to COD4 by ^3Forgive", 15 );
